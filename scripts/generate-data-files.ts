...
const root = new URL("../data-files/", import.meta.url);
...
async function run() {
  ...
  const rootEntries: Entry[] = await rootResponse.json() as Entry[];

  await mkdir(root, { recursive: true });

  async function processDirectory(entry: Entry, basePath: string) {
    const dirResponse = await fetch(`https://unicode-proxy.mojis.dev/proxy/emoji/${entry.path}`);
    const dirEntries: Entry[] = await dirResponse.json() as Entry[];
    ...
    await Promise.all(
      fileEntries.map(async (fileEntry) => {
        ...
        if (fullPath.includes("latest") || fullPath.includes("draft")) {
          return;
        }
        ...
        if (!version) {
          throw new Error(`version is undefined, ${fullPath}`);
        }
  ...
  await Promise.all(directories.map((dir) => processDirectory(dir, `emoji/${dir.path}/`)));
